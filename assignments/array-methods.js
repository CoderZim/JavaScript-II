...
//let allCaps = [];
const allCaps= runners.map(function(runners){
...
// Problem 1: Skinix paid for donations for 5 employees to run but only received paperwork from 3. Verify how many Skinix employees registered for the race.
let companyName= runners.filter(function(runners) {
    return runners.company_name === "Skinix";
});
console.log(companyName);

// Problem 2: Event planners want to send confirmation emails to all runners. Get a list for them.
const emails=[];
    
runners.forEach ((runners)=>{
    emails.push(runners.email);

});
console.log(emails);
// Problem 3 Runner last names need to be capitalized for the signs to go on the back of their shirts.
let backSign=[]
